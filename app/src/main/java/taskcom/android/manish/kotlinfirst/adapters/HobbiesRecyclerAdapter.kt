    ...
    inner class HobbiesViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        init {
            itemView.setOnClickListener {
                ...
                currentHobby?.let {
                    context.showToast("${currentHobby!!.title}  ${context.resources.getString(R.string.is_clicked)}")
                }
            }
            ...
            itemView.imgShare.setOnClickListener {
                ...
                // let{...} block will only execute if currentHobby object is not null
                currentHobby?.let {
                    val message: String = "${context.resources.getString(R.string.my_hobby_is)} : ${currentHobby!!.title}"    // Don't use directly within init{} block
                    val shareIntent = Intent()
                    shareIntent.action = Intent.ACTION_SEND
                    shareIntent.type = "text/plain"
                    shareIntent.putExtra(Intent.EXTRA_TEXT, message)
                    context.startActivity(Intent.createChooser(shareIntent, "${context.resources.getString(R.string.please_select_app)}"))
                }
            }
        }
